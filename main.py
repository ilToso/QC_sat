...
class Satellite:
    ...
    def get_neighbor_distance(self):
        return self.neighbors_distance.keys(), self.neighbors_distance.values()
    ...
    def get_neighbor_qubit_distance(self):
        return self.neighbors_qubit_distance.keys(), self.neighbors_qubit_distance.values()
...
class Constellation:
    def __init__(self, n_sat=10, n_dir=3, radius=1, min_distance=0.1, max_distance=2) -> None:
        ...
        for sat_idx_1, sat_1 in self.constellation.items():
            for sat_idx_2, sat_2 in self.constellation.items():
                ...
                if distance <= radius and sat_1 != sat_2:
                    sat_1.add_neighbor_distance(sat_idx_1 + '-' + sat_idx_2, distance)
                    sat_2.add_neighbor_distance(sat_idx_2 + '-' + sat_idx_1, distance)
    ...
    def draw_constellation(self):
        ...
        for idx, sat in self.constellation.items():
            couple, _ = sat.get_neighbor_distance()
            seen = []
            for cpl in couple:
...
if __name__ == '__main__':
    ...
    radius = 0.2                    # Within this radius, a satellite is considered a neighboring
